  ...
  Future<List<IdentifiedBook>> identifyBooks(
    ...
    try {
      ...
      if (response.statusCode != 200) {
        print('BookIdentificationService: HTTP ${response.statusCode} — ${response.body}');
        return [];
      }

      final decoded = jsonDecode(response.body);
      final List<dynamic> booksJson = (decoded is Map && decoded['books'] is List)
          ? decoded['books'] as List<dynamic>
          : [];

      return booksJson
          .whereType<Map>()
          .map((b) => IdentifiedBook(
      ...
      print('BookIdentificationService: request failed: $e');
      return [];
    }
  }
}